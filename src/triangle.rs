...
impl Triangle {
    ...
    pub fn draw<T: ColorSpace + Copy>(
        ...
        let bb_min_x = self.vector_a.x.min(self.vector_b.x).min(self.vector_c.x);
        let bb_max_x = self.vector_a.x.max(self.vector_b.x).max(self.vector_c.x);
        let bb_min_y = self.vector_a.y.min(self.vector_b.y).min(self.vector_c.y);
        let bb_max_y = self.vector_a.y.max(self.vector_b.y).max(self.vector_c.y);
        ...
        for y in bb_min_y..=bb_max_y {
            for x in bb_min_x..=bb_max_x {
                let current_point = Vector3 { x, y, z: 0 };
                let alpha: f64 =
                ...
                if let Some(z_buffer) = z_buffer_opt.as_deref_mut()
                    && let Some(z_depth) = z_buffer.get_pixel(x_unsigned, y_unsigned)
